# PACo calculations and plotting
...
Host_Sp <-ape::read.tree(file="~/Desktop/R/1000.rumen.tree.nwk")
...
# subsampling trees
df = metadata %>%
  mutate(sample = metadata$Sample_ID) %>%
  dplyr::select(Sample_ID, scientific_name) 
...
D_links_list=read.table("~/Desktop/R/residuals.txt", header=TRUE)
D_links_list
...
write.table(D_links_list, file="~/Desktop/R/D_links_l_tax.txt")
